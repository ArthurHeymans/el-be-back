...
SCALED placements resize their image with the cell canvas; natural-size images
retain their native pixel dimensions."
      ...
      (if-let* ((upload (and continuation
        ...
        (setf (ebb-graphics-state-upload state)
              (make-ebb-graphics-upload
               :params params
               :chunks (unless (string-empty-p decoded) (list decoded))
      ...
      (pcase-let* ((`(,columns ,rows ,pixel-width ,pixel-height)
        ...
        (when (and (> columns 0) (> rows 0)
          ...
          (make-ebb-graphics-placement
           ...
           :scaled (or (> (ebb-graphics--param-integer params ?c 0) 0)
                       (> (ebb-graphics--param-integer params ?r 0) 0))
  ...
  (pcase (ebb-graphics--load-image
    ...
    (`(,final-params ,format ,width ,height ,data
     ...
     (let* ((prospective-id
       ...
       (cond
        ;; Continuations carry only m/q, so dispatch initially defaults to t.
        ;; Honor the original query action before admitting any stored image.
        ((eq (ebb-graphics--param-char final-params ?a ?t) ?q)
