...
(defun ebb-io--report-pixel-size (io process tty helper height width cell)
  "Run IO's pixel-size HELPER for PROCESS on TTY using HEIGHT and WIDTH.
CELL is the normalized cell size captured when the resize was scheduled.
Wait for the helper to finish before allowing another resize: an asynchronous
TIOCSWINSZ could otherwise restore stale rows and columns after a newer resize."
  (setf (ebb-io-pixel-size-timer io) nil)
  (when (and (not (ebb-io-pixel-size-error io))
    ...
    (let ((default-directory temporary-file-directory))
      (condition-case error
          (let ((status
                 (apply #'call-process (car helper) nil nil nil
                        (append (cdr helper)
                                (list tty
            ...
            (unless (eql status 0)
              (ebb-io--disable-pixel-size
               io (format "helper failed: %s" status))))
        (error
