...
(ert-deftest ebb-test-kitty-empty-chunks-do-not-retain-memory ()
  "Empty continuations do not grow the pending upload's chunk list."
  (ebb-test-with-screen (:width 20 :height 6)
    (ebb-test-output parser "\e_Ga=t,f=24,s=1,v=1,i=8,m=1;AQID\e\\")
    (dotimes (_ 1000)
      (ebb-test-output parser "\e_Gm=1;\e\\"))
    (let ((upload (ebb-graphics-state-upload (ebb-screen-graphics screen))))
      (should (= 1 (length (ebb-graphics-upload-chunks upload))))
      (should (= 3 (ebb-graphics-upload-byte-count upload))))
    (ebb-test-output parser "\e_Gm=0;\e\\")
    (should (gethash 8 (ebb-graphics-state-images (ebb-screen-graphics screen))))))

(ert-deftest ebb-test-kitty-chunked-query-preserves-stored-images ()
  "A chunked query neither stores data nor replaces existing placements."
  (ebb-test-with-screen (:width 20 :height 6)
    (ebb-test-output parser "\e_Ga=T,f=24,s=1,v=1,i=8,C=1;AAAA\e\\")
    (let* ((graphics (ebb-screen-graphics screen))
           (image (gethash 8 (ebb-graphics-state-images graphics)))
           (placements (ebb-graphics-state-placements graphics))
           (generation (ebb-graphics-state-generation graphics))
           responses)
      (setf (ebb-parser-write-fn parser) (lambda (s) (push s responses)))
      (dolist (id '(8 9))
        (ebb-test-output parser (format "\e_Ga=q,f=24,s=1,v=1,i=%d,m=1;AQI=\e\\" id))
        (ebb-test-output parser "\e_Gm=0;Aw==\e\\"))
      (should (equal responses '("\e_Gi=9;OK\e\\" "\e_Gi=8;OK\e\\")))
      (should (eq image (gethash 8 (ebb-graphics-state-images graphics))))
      (should-not (gethash 9 (ebb-graphics-state-images graphics)))
      (should (eq placements (ebb-graphics-state-placements graphics)))
      (should (= generation (ebb-graphics-state-generation graphics))))))

(ert-deftest ebb-test-kitty-placement-cursor-respects-origin-margins ()
  "Graphics advance the cursor without applying origin margins twice."
  (ebb-test-with-screen (:width 20 :height 10)
    (ebb-test-output parser "\e[3;9r\e[?69h\e[4;18s\e[?6h\e[1;1H")
    (should (equal '(3 . 2) (ebb-test-cursor screen)))
    (ebb-test-output parser "\e_Ga=T,f=24,s=1,v=1,c=1,r=1;AAAA\e\\")
    (should (equal '(4 . 2) (ebb-test-cursor screen)))))
...
(ert-deftest ebb-test-render-kitty-half-wide-cell-keeps-grid ()
  "Covering either half of a wide glyph preserves adjacent text columns."
  (dolist (history '(nil t))
    (dolist (column '(0 1))
      (dolist (text '("中X" "中\u0301X"))
        (ebb-test-with-screen (:width 6 :height 2)
          (ebb-test-output parser text)
          (ebb-screen-cursor-goto screen 0 column)
          (ebb-test-output parser "\e_Ga=T,f=24,s=1,v=1,i=1,c=1,r=1,C=1;AQID\e\\")
          (when history (ebb-test-output parser "\r\nx\r\n"))
          (let* ((render (make-ebb-render-state :screen screen :graphics-generation -1))
                 (line (if history (ebb-screen-history-render-row screen 0)
                         (ebb-screen-get-line screen 0)))
                 (string (if history (ebb-render--line-to-string-scrollback line 6)
                           (ebb-render--line-to-string line 6))))
            (cl-letf (((symbol-function 'display-graphic-p) (lambda (&rest _) t))
              ...
              (let ((row (ebb-render--apply-graphics render 0 string history)))
                (should (= ?X (aref row 2)))
                (should (get-text-property column 'display row))
                (should-not (get-text-property (- 1 column) 'display row))
                (should-not (get-text-property column 'invisible row))
                (should-not (get-text-property 2 'display row))))))))))

(ert-deftest ebb-test-render-kitty-cell-resize-rescales-explicit-box ()
  "Font changes resize explicit boxes, including virtual placements."
  (dolist (virtual '(0 1))
    (ebb-test-with-screen (:width 6 :height 2)
      (ebb-test-output parser
                       (format "\e_Ga=T,f=24,s=1,v=1,i=1,c=2,r=1,U=%d,C=1;AQID\e\\" virtual))
      (let* ((graphics (ebb-screen-graphics screen))
             (image (gethash 1 (ebb-graphics-state-images graphics)))
             (placement (car (ebb-graphics-state-placements graphics)))
             (render (make-ebb-render-state
                      :screen screen :graphics-cache (make-hash-table :test #'equal)))
             (cell '(8 . 16))
             svg)
        (cl-letf (((symbol-function 'ebb-render-cell-pixel-size) (lambda (_) cell))
                  ((symbol-function 'ebb-render--graphics-raw-png-helper) (lambda (_) "png"))
                  ((symbol-function 'image-type-available-p) (lambda (_) t))
                  ((symbol-function 'create-image)
                   (lambda (data &rest _) (setq svg data) 'mock-image)))
          (dolist (size '((8 . 16) (16 . 32) (4 . 8)))
            (setq cell size)
            (ebb-render--graphics-image-object render image placement)
    ...
    (cl-letf (((symbol-function 'set-process-window-size)
              ...
              ((symbol-function 'call-process)
               (lambda (program _in _out _display &rest args)
                 (cl-incf spawns)
                 (setq helper-command (cons program args))
                 0)))
...
(ert-deftest ebb-test-io-pixel-size-failure-disables-retries ()
  ...
  (let ((io (make-ebb-io :process 'pty :render 'render))
    ...
    (cl-letf (((symbol-function 'process-live-p) (lambda (_) t))
      ...
      (ebb-io--report-pixel-size io 'pty "/dev/pts/1" '("helper")
                                 20 80 '(10 . 20)))
...
(ert-deftest ebb-test-wide-wrap-between-line-renditions ()
  "Wide autowrap uses destination logical width and physical cell storage."
  (dolist (rendition '("\e#6" "\e#3" "\e#4"))
    (ebb-test-with-screen (:width 4 :height 3)
      (ebb-test-output parser (concat rendition "\e[1;2H界"))
      (should (= 1 (ebb-screen-cursor-y screen)))
      (should (= 2 (ebb-screen-cursor-x screen)))
      (should-not (ebb-screen-pending-wrap screen))
      (should (= 4 (length (ebb-line-cells (ebb--line-at screen 1)))))
      (should (= ?界 (ebb-cell-char (aref (ebb-line-cells (ebb--line-at screen 1)) 0)))))
    (ebb-test-with-screen (:width 4 :height 3)
      (ebb-test-output parser (concat "\e[2;1H" rendition "\e[1;4H界"))
      (should (= 1 (ebb-screen-cursor-y screen)))
      (should (= 1 (ebb-screen-cursor-x screen)))
      (should (ebb-screen-pending-wrap screen))
      (should (= 4 (length (ebb-line-cells (ebb--line-at screen 1)))))
      (ebb-test-output parser "X")
      (should (equal "X" (ebb-test-display-line screen 2))))))

(ert-deftest ebb-test-alt-resize-line-rendition-cursor ()
  "Active and retained alternate grids resize cursors in logical columns."
  (dolist (rendition '("\e#6" "\e#3" "\e#4"))
    (dolist (inactive '(nil t))
      (dolist (width '(6 11 12))
        (ebb-test-with-screen (:width 10 :height 3)
          (ebb-test-output parser (concat "\e[?47h" rendition "12345"))
          (should (ebb-screen-pending-wrap screen))
          (when inactive (ebb-test-output parser "\e[?47l"))
          (ebb-screen-resize screen width 3)
          (when inactive (ebb-test-output parser "\e[?47h"))
          (should (= (ebb-screen-cursor-x screen)
                     (pcase width (6 2) (11 4) (12 5))))
          (should (eq (ebb-screen-pending-wrap screen) (< width 12)))
          (should (< (ebb-screen-cursor-x screen)
                     (ebb-screen-line-width screen))))))))
