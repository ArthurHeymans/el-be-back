...
image dimensions rather than stretching them over the propertized text run."
  (let* ((cell (or (ebb-render-cell-pixel-size render)
         ...
         (height (* (or (ebb-graphics-placement-box-rows placement)
                    ...
                    cell-height))
         (geometry
          (and (ebb-graphics-placement-scaled placement)
               (ebb-graphics--placement-geometry
                image (/ width cell-width) (/ height cell-height)
                (cons cell-width cell-height))))
         (draw-width (if geometry (nth 2 geometry)
                       (ebb-graphics-placement-pixel-width placement)))
         (draw-height (if geometry (nth 3 geometry)
                        (ebb-graphics-placement-pixel-height placement)))
         ...
         (cached (if (> (* width height 4) ebb-kitty-graphics-surface-limit)
                     :failed
  ...
  (let ((indices (make-vector (1+ width) (length string)))
    ...
    (while (and cells (< column width) (< position limit))
      (let* ((cell (aref cells column))
             ...
             (carrier (get-text-property position 'ebb-graphics-cell-carrier
                                         string))
             (combining-length (if carrier 0
                                 (length (or (ebb-cell-combining cell) ""))))
             (compact-wide
              (and (not carrier) scrollback
...
invisible viewport spacers so an image on the second cell is actually visible."
  (let ((cells (and line (ebb--line-ensure-cells line width)))
        (column 0)
        (position 0)
        parts)
    (while (and cells (< column width))
      (let* ((span (max 1 (ebb-cell-width (aref cells column))))
             (end-column (min width (+ column span))))
        (when (and (> span 1)
                   (< (aref indices column) (length string))
                   (cl-loop for col from column below end-column
                            thereis (aref owners col)))
          (let* ((start (aref indices column))
                 (end (min (length string) (aref indices end-column)))
                 (blank (make-string (- end-column column) ?\s)))
            (when (< start (length string))
              (set-text-properties 0 (length blank)
                                   (text-properties-at start string) blank))
            (remove-text-properties
             0 (length blank)
             '(display nil invisible nil ebb-cell-width nil ebb-wide-spacer nil)
             blank)
            (put-text-property 0 (length blank) 'ebb-graphics-cell-carrier t blank)
            (push (substring string position (min start (length string))) parts)
            (push blank parts)
            (setq position end)))
        (setq column end-column)))
    (if parts
        (apply #'concat (nreverse (cons (substring string position) parts)))
      string)))
